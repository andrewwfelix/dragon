...
async function fixTraitFormatting() {
  console.log('Fetching monsters with special traits...');
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      for (const monster of batch) {
        ...
        const fixedTraits = monster.special_traits.map(trait => {
          ...
          // Fix trait description formatting
          if (trait.description && typeof trait.description === 'string') {
            const originalDesc = trait.description;
            // Replace patterns like "Name:Description" with "Name: Description"
            fixedTrait.description = trait.description.replace(/:([A-Za-z])/g, ': $1');
            
            if (fixedTrait.description !== originalDesc) {
              hasChanges = true;
              console.log(`  Fixed trait description: "${originalDesc}" -> "${fixedTrait.description}"`);
            }
          }
    ...
    console.log(`\n✅ Fixed formatting for ${fixedCount} monsters`);
    console.log('Trait formatting fix completed!');

  } catch (error) {
    console.error('Error in fixTraitFormatting:', error);
  }
}
