-- D&D 5e Application Database Schema for Supabase
...
-- Make reference data publicly readable
CREATE POLICY "Public can read monsters" ON monsters FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read spells" ON spells FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read magic_items" ON magic_items FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read classes" ON classes FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read races" ON races FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read backgrounds" ON backgrounds FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read feats" ON feats FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read weapons" ON weapons FOR SELECT TO anon USING (true);
CREATE POLICY "Public can read armor" ON armor FOR SELECT TO anon USING (true);

-- Insert some sample data to test
INSERT INTO classes (name, slug, hit_die, data) VALUES 
('Fighter', 'fighter', 10, '{"name": "Fighter", "hit_die": 10}'),
('Wizard', 'wizard', 6, '{"name": "Wizard", "hit_die": 6}'),
('Cleric', 'cleric', 8, '{"name": "Cleric", "hit_die": 8}')
ON CONFLICT (slug) DO NOTHING;

INSERT INTO races (name, slug, is_subrace, data) VALUES 
('Human', 'human', false, '{"name": "Human", "traits": []}'),
('Elf', 'elf', false, '{"name": "Elf", "traits": []}'),
('Dwarf', 'dwarf', false, '{"name": "Dwarf", "traits": []}')
ON CONFLICT (slug) DO NOTHING;

-- Success message
SELECT 'D&D 5e Database schema created successfully!' as message; 